    ...
    <div className="space-y-6">
      ...
      <div className="grid grid-cols-2 gap-4">
        <div className="bg-white p-5 rounded-3xl shadow-sm border border-slate-100 text-center">
          <span className="block text-3xl font-bold text-slate-800">{currentWeight} <span className="text-sm font-normal text-slate-400">kg</span></span>
          <span className="text-[10px] font-bold text-slate-400 uppercase tracking-widest mt-1">Peso Atual</span>
        </div>
        <div className="bg-white p-5 rounded-3xl shadow-sm border border-slate-100 text-center">
          <span className="block text-3xl font-bold text-slate-800">{exerciseCount}</span>
          <span className="text-[10px] font-bold text-slate-400 uppercase tracking-widest mt-1">Dias Treinados</span>
        </div>
      </div>
      ...
      <div className="bg-white p-6 rounded-3xl shadow-sm border border-slate-100">
        ...
        <div className="flex gap-2">
          <input 
            ...
            placeholder="00.0"
            className="flex-1 bg-slate-50 border-none rounded-xl px-4 py-3 outline-none text-slate-700 focus:ring-2 focus:ring-emerald-500"
          />
          <button 
            onClick={() => {
              if (newWeight) {
      ...
      <div className="bg-white p-6 rounded-3xl shadow-sm border border-slate-100">
        ...
        <div className="space-y-3 max-h-48 overflow-y-auto hide-scrollbar">
          {exerciseLogs.length === 0 ? (
            <p className="text-slate-300 text-sm italic text-center py-2">Nenhum treino registrado.</p>
          ) : (
            exerciseLogs.map((log, idx) => (
              <div key={idx} className="flex justify-between items-center text-sm py-2 group border-b border-slate-50 last:border-none">
                <span className="text-slate-500 font-medium">{new Date(log.date).toLocaleDateString('pt-BR')}</span>
                <div className="flex items-center gap-3">
                  <span className="bg-orange-100 text-orange-600 px-2 py-0.5 rounded text-[10px] font-bold uppercase">Treino OK</span>
                  <button 
      ...
      <div className="bg-white p-6 rounded-3xl shadow-sm border border-slate-100">
        <h3 className="font-bold text-slate-700 mb-4">Histórico de Peso</h3>
        <div className="space-y-3 max-h-48 overflow-y-auto hide-scrollbar">
          {userWeights.length === 0 ? (
            <p className="text-slate-300 text-sm italic text-center py-2">Nenhum registro ainda.</p>
          ) : (
            userWeights.map((log, idx) => (
              <div key={idx} className="flex justify-between items-center text-sm py-2 border-b border-slate-50 last:border-none">
