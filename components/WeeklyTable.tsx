          ...
          return (
            <div key={dayName} className="relative">
              <div className="flex items-center gap-3 mb-4">
                ...
                <button 
                  onClick={async () => {
                    if (confirm(`Copiar cardápio de ${copyLabel} para ${dayName}? Isso removerá o que você já registrou em ${dayName}.`)) {
                      await copyDayMenu(dayIdx, selectedWeek);
                    }
                  }}
                  title={`Copiar de ${copyLabel}`}
              ...
              <div className="grid gap-3">
                {TIMES.map(time => {
                  ...
                  return (
                    <div key={time} className={`flex items-start gap-4 p-3 rounded-2xl border transition-all ${meals.length > 0 ? 'bg-white border-slate-100 shadow-sm' : 'bg-slate-50/50 border-dashed border-slate-200 opacity-60'}`}>
                      ...
                      <div className="flex-1 space-y-2">
                        {meals.length > 0 ? (
                            ...
                            <div key={meal.id} className="flex justify-between items-center group">
                              <div className="flex items-center gap-3">
                                <button 
                                  ...
                                  className={`w-5 h-5 rounded border-2 flex items-center justify-center transition-all shrink-0 ${meal.consumed ? (activeUser === 'Thiago' ? 'bg-sky-500 border-sky-500 text-white' : 'bg-rose-500 border-rose-500 text-white') : 'border-slate-200 text-transparent'}`}
                                >
                                ...
                                <div>
                                  <p className={`text-sm font-semibold leading-tight ${meal.consumed ? 'text-slate-400 line-through' : 'text-slate-700'}`}>{meal.food}</p>
                                  <p className="text-[10px] text-slate-400">{meal.amount}</p>
                                </div>
                              </div>
                              <div className="flex items-center gap-2">
                                <span className={`text-[10px] font-bold ${meal.consumed ? accentColor : 'text-slate-400'}`}>{meal.calories} kcal</span>
                          ...
                          <p className="text-[10px] text-slate-300 italic pt-1 uppercase tracking-tighter">Vazio</p>
                        )}
                      </div>
                    </div>
                  );
